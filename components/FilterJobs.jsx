...
const FilterJobs = () => {
    ...
    return(
    <div>
      <h2 className='text-sm text-base-400 mb-2 opacity-40'>Filter jobs based on</h2>
      <form className='flex sm:flex-row flex-col gap-4'>
        <div className='join'>
            <label htmlFor="company" className='bg-base-100 join-item  flex justify-center align-middle text-center items-center pl-5 w-[50%]'>
            ...
            <input 
              ...
              name='company'
              required 
              className='join-item input bg-base-300 w-full'
              value={searchParams.get('companyName') ?? '' } 
              onChange={(e)=>handleFilterQuery('companyName',e.currentTarget.value)}/>
        </div>
        <div className='join'>
            <label htmlFor="role" className='bg-base-100 join-item flex justify-center align-middle text-center items-center pl-5 w-[50%]'>
              <span className='text-xs mr-5'>Role</span>
            </label>
            <input type='text' name='role' required value={searchParams.get('role') ?? '' }  className='join-item input bg-base-300 w-full' onChange={(e)=>handleFilterQuery('role',e.currentTarget.value)}/>
        </div>
      </form>
    </div>
    )
